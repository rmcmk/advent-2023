...
library ByteSources {
    ...
    function empty() internal pure returns (ByteSource memory) {
        return ByteSource({ data: EMPTY, cursor: 0 });
    }
    ...
    function writeByte(ByteSource memory source, bytes1 value) internal pure {
        ensureAvailable(source, 1);
        source.data[source.cursor++] = value;
    }

    function ensureAvailable(ByteSource memory source, uint256 amount) internal pure {
        if (source.cursor + amount > source.data.length) {
            source.data = abi.encodePacked(source.data, new bytes(amount));
        }
    }
    ...
    function peekByteAt(ByteSource memory source, uint256 index) internal pure returns (bytes1) {
        require(index < source.getLength(), "Index out of bounds");
    ...
    function readUntil(ByteSource memory source, bytes1 target) internal pure returns (ByteSource memory) {
    ...
    function toLines(ByteSource memory source) internal pure returns (ByteSource[] memory lines) {
    ...
    function toBytes1Matrix(ByteSource memory source) internal pure returns (bytes1[][] memory matrix) {
        ByteSource[] memory lines = toLines(source);
        uint256 width = lines[0].getLength();
        ...
        for (uint256 y = 0; y < height; y++) {
            ...
            for (uint256 x = 0; x < width; x++) {
                matrix[y][x] = lines[y].peekByteAt(x);
            }
        }
    }

    function parseUint(ByteSource memory source) internal pure returns (uint256) {
        return vm.parseUint(source.toString());
    }
    ...
    function indexOf(ByteSource memory source, bytes memory target) internal pure returns (int256) {
        MatchResult memory result = findFirst(source, target);
        if (result.isValid()) {
            return int256(result.startIndex);
        }
        return -1;
    }

    function contains(ByteSource memory source, bytes memory target) internal pure returns (bool) {
        return indexOf(source, target) >= 0;
    }

    function contains(ByteSource memory source, bytes1 target) internal pure returns (bool) {
        return indexOf(source, target) >= 0;
    }

    function findLast(ByteSource memory source, bytes memory targetBytes)
        internal
        pure
